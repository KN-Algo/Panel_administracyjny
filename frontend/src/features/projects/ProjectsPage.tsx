...
type ProjectCardProps = {
  ...
  onSetSlide: (projectKey: string, slideIdx: number) => void;
  onOpenModal: (images: string[], startIndex: number) => void;
  onRefMount: (key: string, el: HTMLDivElement | null) => void;
};
...
function ProjectCard({
  ...
  return (
    <div className="mb-4" ref={(el) => onRefMount(projectKey, el)}>
      <button
                ...
                {project.images!.length === 1 ? (
                  <div className="flex justify-center">
                    <img
                      ...
                      onClick={() => onOpenModal(project.images!, 0)}
                    />
                  </div>
                ) : (
                  <div className="relative">
                    <div className="flex justify-center items-center h-125 bg-gray-50 rounded-2xl">
                      <img
                        src={project.images![slideIndex].replace(
                          "../img/",
                          "/img/",
                        )}
                        ...
                        onClick={() => onOpenModal(project.images!, slideIndex)}
                      />
                    </div>
                    ...
                    <div className="flex justify-center gap-2 mt-4">
                      {project.images!.map((_, idx) => (
                        <button
                          key={idx}
                          onClick={() => onSetSlide(projectKey, idx)}
...
export default function ProjectsPage() {
  ...
  const toggleProject = (index: number) => {
    ...
    if (expandedProject !== null && expandedProject !== index) {
      setExpandedProject(null);
      setTimeout(() => scrollAndExpandProject(index, "current"), 300);
    } else {
  ...
  const toggleMipProject = (index: number) => {
    ...
    if (expandedMipProject !== null && expandedMipProject !== index) {
      setExpandedMipProject(null);
      setTimeout(() => scrollAndExpandProject(index, "mip"), 300);
    } else {
  ...
  const handleSetSlide = (projectKey: string, slideIdx: number) => {
    setCurrentSlide((prev) => ({ ...prev, [projectKey]: slideIdx }));
  };
  ...
  const handleRefMount = (key: string, el: HTMLDivElement | null) => {
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          {projects.map((project, index) => (
            <ProjectCard
              ...
              projectType="current"
              slideIndex={currentSlide[`current-${index}`] || 0}
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          {mipProjects.map((project, index) => (
            <ProjectCard
              ...
              project={project}
              index={index}
              isExpanded={expandedMipProject === index}
              onToggle={toggleMipProject}
              projectType="mip"
              slideIndex={currentSlide[`mip-${index}`] || 0}
              onSlideChange={handleSlideChange}
              onSetSlide={handleSetSlide}
              onOpenModal={openModal}
              onRefMount={handleRefMount}
